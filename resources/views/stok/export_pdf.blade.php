    ...
    <table class="border-bottom-header">
        <tr>
            <td width="15%" class="text-center">
                <img src="{{ asset('polinema-bw.png') }}" width="100">
            </td>
            <td width="85%">
        ...
        <tbody>
            @foreach($stok as $b)
                <tr>
                    ...
                    <td class="text-center">{{ $b->stok_id }}</td>
                    <td>{{ $b->barang->nama_barang }}</td>
                    <td>{{ $b->user->nama }}</td>
                    <td class="text-center">{{ $b->stok_tanggal }}</td>
                    <td class="text-center"> {{ number_format($b->stok_jumlah) }} </td>
                </tr>
            @endforeach
        </tbody>
    </table>
